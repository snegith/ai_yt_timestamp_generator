...
def _build_embeddings_for_similarities(similarities: list[float], dim: int = 4) -> np.ndarray:
    ...
    equals similarities[i] for each i, with every embedding a unit vector.
    """
...
def test_boundary_threshold_iff_similarity_below_adaptive_cutoff(
    ...
    **Validates: Requirements 8.3**
    ...
    For a sequence long enough to analyse, detect_boundaries() includes
    ...
    for i, sim in enumerate(actual_sims):
        ...
        if should_include:
            assert id(window) in result_set, (
                f"Window {i + 1} should be included (similarity={sim:.6f} < "
                f"cutoff={cutoff:.6f}), but was not found in result"
            )
        else:
            assert id(window) not in result_set, (
                f"Window {i + 1} should NOT be included (similarity={sim:.6f} >= "
                f"cutoff={cutoff:.6f}), but was found in result"
            )
...
def test_uniform_similarity_only_first_window_returned() -> None:
    ...
    assert result == [windows[0]], (
        f"With identical similarities, only windows[0] should be returned, "
